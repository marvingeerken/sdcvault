{%- macro derive_columns(source_relation=none, columns=none) -%}
    ...
    {{- adapter.dispatch('derive_columns', 'sdcvault')(source_relation=source_relation, columns=columns) -}}
...
{%- if columns is mapping and columns is not none -%}
    ...
    {%- for derived_column in columns -%}
        ...
        {%- if datavault4dbt.is_list(column_config) -%}
            {%- set column_list = [] -%}

            {%- for concat_component in column_config -%}
                {%- set column_str = sdcvault.as_constant(concat_component) -%}
        ...
        {%- else -%}
            {%- if column_config is mapping and column_config -%}
                ...
                {%- if datavault4dbt.is_list(column_config['source_column']) -%}
                    {%- set column_list = [] -%}

                    {%- for concat_component in column_config['source_column'] -%}
                        {%- set column_str = sdcvault.as_constant(concat_component) -%}
                        {%- if column_escape is true %}
                            {%- set column_str = datavault4dbt.escape_column_names(column_str) -%}
                        {% endif %}
                        {%- do column_list.append(column_str) -%}
                    {%- endfor -%}
                    ...
                    {%- do der_columns.append(concat_string) -%}
                {%- else -%}
                    {%- set column_str = sdcvault.as_constant(column_config['source_column']) -%}
                    {%- if column_escape is true -%}
                        {%- do der_columns.append(datavault4dbt.escape_column_names(column_str) ~ " as " ~ derived_column) -%}
                    {%- else -%}
                        {%- do der_columns.append(column_str ~ " as " ~ derived_column) -%}
                    {%- endif -%}
                {%- endif -%}
            {%- else -%}
                {%- set column_str = sdcvault.as_constant(column_config) -%}
                {%- do der_columns.append(column_str ~ " as " ~ derived_column) -%}
            {%- endif -%}
        {%- endif -%}
