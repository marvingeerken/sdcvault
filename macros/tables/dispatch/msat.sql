{#
    ...
    In most cases we have particular columns, which define the multi-activity in our MSATs. Usually they define the Primary Key in the source.
    We can add those to our MSAT Primary Key to load them into our Raw Vault.

    The only thing that is missing is the is_delete detection on those PK combination, since our ESATs work on our Hub HK without MA-Key.
    To solve this the delete detection of HK+MA-Key, its done directly inside the MSAT doing the load.
