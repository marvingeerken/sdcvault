{%- macro default__nlink(source_models, link_hash_key, foreign_hash_keys, src_payload, src_ldts, src_rsrc,
                         high_water_mark, multi_batch_bool) -%}
...
{%- set source_model_values = fromjson(datavault4dbt.source_model_processing(source_models=source_models, parameters={'link_hk':link_hash_key}, foreign_hashkeys=foreign_hash_keys, payload=src_payload)) -%}
...
rsrc_static_union as (
            {#- Create one unionized table over all sources. It will be the same as the already existing
               nlink, but extended by the rsrc_static column. #}
            {%- for source_model in source_models %}
                {%- set source_number = source_model.id | string %}
...
src_new_{{ source_number }} as (
    ...
    select
        ...
        src.{{ src_ldts }},
        src.{{ src_rsrc }},
        {{ datavault4dbt.print_list(source_model['payload']) }}
    ...
    {%- elif is_incremental() and source_models | length == 1 and not ns.has_rsrc_static_defined and not high_water_mark is true %}
    where src.{{ src_ldts }} > (
        select max({{ src_ldts }})
        from {{ this }}
    )
